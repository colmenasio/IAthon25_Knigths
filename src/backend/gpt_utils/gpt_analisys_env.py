...
class GptAnalisysEnv:
    ...
    def __init__(self, user_prompt):
        ...
        Once you are done you can signal it by executing the `exit()` function whit no arguments.\n\n
    ...
    def _parse_executable(self, message: str):
        if "```python" in message:
            return message.split("```python")[1].split("```", 1)[0]
        else:
            return message
    ...
    def analyze_dataset(self, do_log: bool = False):
        ...
        while True:
            ...
            local_context= {"df": df, "pd": pd, "np": np, "scipy": scipy,"stats": stats}
            command_output = self._execute_and_capture(executable_code, local_context) 
            if do_log: print(f"Std Output: {command_output}")
            ...
            if do_log: print(f"Response: {assistant_response}")
            if self._is_quit_request(assistant_response): break
